...
int ArmClient::start_socket() {
    ...
    s = getaddrinfo(SERVER_IP, std::to_string(SERVER_PORT).c_str(), &hints, &res);
    if (s != 0) {
    ...
    for (rp = res; rp != NULL; rp = rp->ai_next) {
        this->clientSocket = socket(rp->ai_family, rp->ai_socktype, rp->ai_protocol);
...
int ArmClient::setup() {
    std::cout << "ArmClient setup called" << std::endl;
    return this->start_socket();
}
